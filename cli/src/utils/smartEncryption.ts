...
export async function encryptFunctionsInFile(content: string, ext: string, filepath: string): Promise<string> {
...
export async function decryptFunctionsInFile(content: string, ext: string, filepath: string): Promise<string> {
  // Map extension to language
  const language = EXTENSION_MAP[ext];
  if (!language || !LANGUAGE_PATTERNS[language]) return content;
...
export async function decryptCodeBlock(encrypted: string, key: Buffer): Promise<string> {
