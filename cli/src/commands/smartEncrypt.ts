...
async function encryptFunctions(content: string, language: string, filepath: string): Promise<string> {
...
async function decryptFunctions(content: string, language: string, filepath: string): Promise<string> {
  const key = await deriveGitHubKey(filepath);
